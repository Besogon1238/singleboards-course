...
Для комфортной работы с C/C++ проектами в VS Code рекомендуется установить следующие расширения:

- C/C++ Runner — обеспечивает базовую поддержку языка, подсветку синтаксиса и навигацию по коду
- Remote — SSH — для удаленной разработки (потребуется в дальнейшем)
- clangd — для обеспечения подсветки синтаксиса и навигации по коду
...
Для обеспечения подсветки синтаксиса и навигации по коду рекомендуется установить расширение *Clangd* на удаленной машине. На целевой плате необходимо установить соответствующие пакеты:

```
# apt-get install clang clangd
```

После активации расширения (Ctrl+Alt+P > Clangd: Activate) выполняется индексация проекта, и становятся доступными функции перехода к определениям, контекстной подсказки и другие возможности.
...
Чтобы устранить указанные причины необходимо внести изменения в файлы **c_cpp_properties.json** и **settings.json** из директории .vscode, которая создается после первой попытки собрать приложение.

Приведем в соответствие поле *compilerPath* из файла **c_cpp_properties.json**, указывающее путь к компилятору:
