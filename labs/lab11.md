...
Одним из важных преимуществ программирования микроконтроллеров является детерминированное выполнение операций — время выполнения каждой инструкции известно точно, что критически важно для систем реального времени. Однако это достигается за счёт ограниченных ресурсов: микроконтроллеры имеют мало памяти (обычно килобайты, а не гигабайты) и работают на сравнительно низких частотах процессора (мегагерцы вместо гигагерц).
...
**Цифровые входы/выходы (GPIO)** — пины общего назначения, которые могут работать как:
- **Выходы** — подавать напряжение (HIGH/LOW) для управления устройствами
- **Входы** — читать напряжение для получения данных от датчиков

**Прерывания (interrupts)** — механизм реагирования на внешние события:
- Аппаратные прерывания — от кнопок, таймеров, коммуникационных интерфейсов
- Программные прерывания — вызовы специальных функций

## Практическая часть

### Подготовка необходимого оборудования

1. **Arduino Uno/Mega** — подключите к компьютеру через USB
...
2. Выберите правильную плату: **Инструменты → Плата → Arduino Uno/Mega**
...
**Пины 2 и 3 на Arduino Uno** или **пины 2,3,18,19,20 и 21 на Arduino Mega** могут быть использованы для работы с аппаратными прерываниями.
...
![Пример](../pictures/example_connect.jpg)
